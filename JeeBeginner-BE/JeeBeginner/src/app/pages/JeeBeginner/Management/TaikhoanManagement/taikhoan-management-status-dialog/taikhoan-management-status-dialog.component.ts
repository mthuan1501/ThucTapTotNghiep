...
import { TaikhoanModel, TaikhoanStatusModel } from '../Model/taikhoan-management.model';
import { TranslateService } from '@ngx-translate/core';
import { PartnerFilterDTO } from '../../PartnerManagement/Model/partner-management.model';

@Component({
  selector: 'app-taikhoan-management-status-dialog',
  templateUrl: './taikhoan-management-status-dialog.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class TaikhoanManagementStatusDialogComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<TaikhoanManagementStatusDialogComponent>,
  ...
  loadInitData() {
    if (this.item.RowId !== 0) {
      const sbGet = this.taikhoanManagementService.getNoteLock(this.item.RowId).pipe(tap((res: ResultObjModel<string>) => {
        if (res.status === 1) {
          this.note = res.data;
  ...
  getTitle() {
    if (this.data.item.IsLock) {
      return this.translateService.instant('TAIKHOANMANAGEMENT.MOKHOATAIKHOAN') + ' ' + this.data.item.Username;
    }
    return this.translateService.instant('TAIKHOANMANAGEMENT.KHOATAIKHOAN') + ' ' + this.data.item.Username;
  }

  private prepareData(): TaikhoanStatusModel {
    let model = new TaikhoanStatusModel();
    model.empty();
  ...
  update(item: TaikhoanStatusModel) {
    this.isLoadingSubmit$.next(true);
    this.taikhoanManagementService.UpdateStatusTaikhoan(item).subscribe((res) => {
