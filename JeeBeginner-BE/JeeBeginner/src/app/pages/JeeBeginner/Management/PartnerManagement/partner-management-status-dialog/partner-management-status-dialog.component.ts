...
@Component({
  selector: 'app-partner-management-status-dialog',
  templateUrl: './partner-management-status-dialog.component.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class PartnerManagementStatusDialogComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<PartnerManagementStatusDialogComponent>,
  ...
  loadInitData() {
    if (this.item.RowId !== 0) {
      const sbGet = this.partnerManagementService.getNoteLock(this.item.RowId).pipe(tap((res: ResultObjModel<string>) => {
        if (res.status === 1) {
          this.note = res.data;
  ...
  update(item: AccountStatusModel) {
    ...
    this.partnerManagementService.UpdateStatus(item).subscribe((res) => {
