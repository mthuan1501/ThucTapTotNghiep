...
const API_PRODUCTS_URL = environment.ApiRoot + "/dm_doitacbaohiem";
...
export class DoiTacBaoHiemManagementService {
  ...
  private findData(tableState: ITableState): Observable<any> {
    ...
    const url = API_PRODUCTS_URL + "/DM_DoiTacBaoHiem_List";
    return this.http
  ...
  DM_DoiTacBaoHiem_Insert(item: DoiTacBaoHiemModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + "/DM_DoiTacBaoHiem_Insert";
    return this.http.post<any>(url, item, { headers: httpHeaders });
  }
  UpdateDoiTacBaoHiem(item: DoiTacBaoHiemModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + "/UpdateDoiTacBaoHiem";
    return this.http.post<any>(url, item, { headers: httpHeaders });
  }
  DeleteDoiTacBaoHiem(item: DoiTacBaoHiemModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + "/DeleteDoiTacBaoHiem";
    return this.http.post<any>(url, item, { headers: httpHeaders });
  }
  ...
  GetDoiTacBaoHiemID(Id_DV: number): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_PRODUCTS_URL + `/GetDoiTacBaoHiemID?Id_DV=${Id_DV}`;
    return this.http.get<any>(url, { headers: httpHeaders });
  }

  SearchDoiTacBaoHiem(TenDoiTacBaoHiem: string): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url =
      API_PRODUCTS_URL +
      `/SearchDoiTacBaoHiem?TenDoiTacBaoHiem=${TenDoiTacBaoHiem}`;
    return this.http.get<any>(url, { headers: httpHeaders });
  }
