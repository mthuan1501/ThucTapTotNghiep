...
export class LayoutInitService {
  ...
  private initSkins() {
    ...
    const headerMenuSelfTheme = this.layout.getProp('header.menu.desktop.submenu.self.theme') || '';
    ...
    if (asideSelfDisplay === false) {
      ...
      document.body.classList.add(`aside-${headerSelfTheme}`);
      document.body.classList.add(`header-base-${headerSelfTheme}`); 
    } else {
      document.body.classList.add(`brand-${brandSelfTheme}`);
      document.body.classList.add(`aside-${asideSelfTheme}`);
      document.body.classList.add(`header-base-${headerSelfTheme}`);
      document.body.classList.add(`header-menu-${headerMenuSelfTheme}`); 
    }
  }
}
